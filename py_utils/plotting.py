...
def plot_heatmaps(
    ...
    Args:
        ...
        axes (plt.Axes): plt.Axes to plot in
        idxs (list, optional): indices of rows from l_info to plot. Defaults to None. In that case,
    ...
    def plot_one_heatmap(df, ax, title, vmin, vmax):
        sns.heatmap(
            df.round(1),
            annot=True,
            linecolor="white",
    ...
    l_map = list(map(lambda x: x.melt(), l_heatmap))
    df_min_max = reduce(lambda x, y: pd.concat([x, y]), l_map)
    vmin, vmax = (df_min_max["value"].min(), df_min_max["value"].max())
    if l_titles == None:
        l_titles = ["Smooth", "Rough"]
    plot_heatmap_partial = partial(plot_one_heatmap, vmin=vmin, vmax=vmax)
    list(map(plot_heatmap_partial, l_heatmap, axes, l_titles))
